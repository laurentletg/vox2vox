...
train_cts = glob.glob(os.path.join(path_train, 'ct', '*.nii.gz'))
train_mrs = glob.glob(os.path.join(path_train, 'mr', '*.nii.gz'))
print(f'CT train: {len(train_cts)}')
print(f'MR train: {len(train_mrs)}')

test_cts = glob.glob(os.path.join(path_test, 'ct', '*.nii.gz'))
test_mrs = glob.glob(os.path.join(path_test, 'mr', '*.nii.gz'))
...
def batch_read_nifti(path, save_path, subfolder='train'):
    ...
    cts = glob.glob(os.path.join(path, subfolder, 'ct', '*.nii.gz'))
    mrs = glob.glob(os.path.join(path, subfolder, 'mr', '*.nii.gz'))
...
if __name__ == '__main__':
    ...
    batch_read_nifti(PATH, SAVE_PATH, subfolder='train')
    batch_read_nifti(PATH, SAVE_PATH, subfolder='test')
